...
void MD_IMU::initialize_md_imu_driver(void){
  CAN_initialize(_250k,"CAN0");  //CAN bit_rate : 250kbps, ID : 0xB7AC01, is_ext_mode : true
}

void MD_IMU::md_write(BYTE data_array[]){
...
void MD_IMU::imu_write(BYTE data_array[]){
...
void MD_IMU::send_RPM(short R_RPM, short L_RPM){
...
void MD_IMU::Data_REQ(BYTE R_PID){
...
struct Encoder_data MD_IMU::read_Encoder(void){
...
void MD_IMU::contol_vel(float *cmd_vel){
...
void MD_IMU::software_reset(void){
  cmd(99);  //센서를 소프트웨어적으로 리셋 시킵니다. 센서의 전원을 껐다 켜는 것과 같습니다.
}

void MD_IMU::set_sync_tx_cycle(int cycle_num){
...
void MD_IMU::set_sync_req(bool onoff){
  //CAN 동기화 데이터 전송 설정
  BYTE cmd_arr[8]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
  cmd_arr[0] = 0x18;
...
void MD_IMU::read_sync_data(void){
 ...
 ***********************************************************/
int MD_IMU::Byte2Int32(BYTE d4, BYTE d5, BYTE d6, BYTE d7)
{
  return ((int)d4 | (int)d5<<8 | (int)d6<<16 | (int)d7<<24);
}


short MD_IMU::Byte2Int16(BYTE d1, BYTE d2)
{
  return (short)((short)d1 | (short)d2<<8);
}

float MD_IMU::Byte2float32(BYTE d4, BYTE d5, BYTE d6, BYTE d7)
{
  float result = 0.0;
